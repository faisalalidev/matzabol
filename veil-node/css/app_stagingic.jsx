...
io.on("connection", function (socket) {
    if (socket.user_id) {
        ...
        io.sockets.emit(
            "socketStatus",
            "Socket connected: " + socket.user_id + " Type: " + socket.type
        );
    ...
    socket.on("sendMessage", function (data, callback) {
        ...
        saveMessage(data.thread_id, data.user_id, data.message, data.uu_id, data.created_at, function (err, result) {
            if (err) {
                ...
                if (data.thread_id) {
                    ...
                    io.sockets.in(data.thread_id).emit("newMessage", data);
...
let saveMessage = function (threadId, senderId, message, uu_id, created_at, callback) {
    let insertQuery = `INSERT INTO chats (thread_id, sender_id, message, uu_id, status, created_at, updated_at) 
                        VALUES (
...
const deleteMessageById = function (socket, message_id, callback) {
    let query = `SELECT deleted_by FROM chats WHERE id = ${db.escape(message_id)};`;
    console.log(query);
    db.query(query, function (err, result) {
        if (!err) {
            console.log(result);
            if (result && result.deleted_by != null) {
                let query2 = `UPDATE chats SET deleted_at = ${db.escape(new Date())} WHERE id = ${db.escape(parseInt(message_id))};`;
                db.query(query2, function (err, result) {
                    callback(err, result);
                });
            }
            else {
                let query3 = `UPDATE chats SET deleted_by = ${db.escape(parseInt(socket.user_id))} WHERE id = ${db.escape(parseInt(message_id))};`;
...
const markMessageAsReceivedByServer = function (chat_id, user_id, thread_id, callback) {
    let insertQuery = `INSERT INTO chat_reads (chat_id, user_id, created_at, updated_at) 
                       SELECT ${db.escape(parseInt(chat_id))}, user_id, ${db.escape(new Date())}, ${db.escape(new Date())} 
                       FROM thread_users 
...
const markMessageAsReceivedByUser = function (chat_id, callback) {
    let query = `UPDATE chat_reads SET 
                        type = ${db.escape('received_by_user')}, 
                        received_by_user = ${db.escape(new Date())},
                        updated_at = ${db.escape(new Date())}
                 WHERE chat_id = ${db.escape(parseInt(chat_id))};`;
...
const markMessageAsReadByUser = function (chat_id, callback) {
    let query = `UPDATE chat_reads crs SET 
                 ...
                 WHERE crs.chat_id = ${db.escape(parseInt(chat_id))};`;
    db.query(query, function (err, result, fields) {
        callback(err, result);
    });
};
...
const getUndeliveredMessages = function (socket, user_id, callback) {
    let query = `SELECT c.*, cr.type AS message_status, u.full_name AS username, u.id AS user_id FROM chats c 
...
const getMessagesStatus = function (socket, messages_id, callback) {
    let query = `SELECT c.id, cr.type AS message_status 
                 ...
                 WHERE c.id IN (${db.escape(messages_id)}) 
                 AND c.deleted_at IS NULL AND (c.deleted_by <> ${db.escape(parseInt(socket.user_id))} OR c.deleted_by IS NULL)`;
    db.query(query, (err, result, fields) => {
        if (!err) {
            callback(result);
...
const messagesReceivedByReceiver = function (message_id, callback) {
    let query = `UPDATE chat_reads cr SET 
                 ...
                 WHERE cr.chat_id IN (${db.escape(message_id)}) AND cr.type <> 'read_by_user';`;
    db.query(query, function (err, result, fields) {
        if (!err) {
            callback(err, result);
        }
...
const messagesReadByReceiver = function (messages_id, callback) {
    let query = `UPDATE chat_reads cr SET 
                        ...
                        cr.updated_at = ${db.escape(new Date())}
                 WHERE cr.chat_id IN (${db.escape(messages_id)}) AND cr.type <> 'read_by_user';`;
    db.query(query, function (err, result, fields) {
        if (!err) {
            callback(err, result);
        }
